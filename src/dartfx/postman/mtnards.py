...
class MtnaRdsPostmanPublisher(BaseModel):
    ...
    def _add_regression_request_parameters(self, item: postman_collection.Item):
        ...
        item.request.url.create_query_parameter(
            "format",
            None,
            "RDS can return a variety of JSON objects to plug into various java script charting libraries. Available formats: MTNA_RDS, MTNA_simple, MTNA_TABULATE, AMCHARTS, GCHARTS, PLOTLY_BAR, PLOTLY_BOXPLOT, PLOTLY_BUBBLE, PLOTLY_HIST, PLOTLY_PIE, PLOTLY_SCATTER.",
            True,
        )
        item.request.url.create_query_parameter(
            "groupby",
            None,
            "When computing a new variable using a function that depends on aggregation, the group by parameter can specify which columns to group by.",
            True,
        )
        item.request.url.create_query_parameter(
    ...
    def _add_select_request_parameters(self, item: postman_collection.Item):
        ...
        item.request.url.create_query_parameter(
            "cols",
            None,
            "Column names, regular expressions, keywords, variable groups, or concepts to select. Any of these can be excluded as well by prepending '~' to these syntaxes.",
            True,
        )
        item.request.url.create_query_parameter(
        ...
        item.request.url.create_query_parameter(
            "lock",
            None,
            "Column names, regular expressions, key words, variable groups, or concepts to lock. These will be returned first adn listed out in DataSetInformation that is returned on the DataSet.",
            True,
        )
        item.request.url.create_query_parameter(
            "metadata", "1", "Flag specifying if metadata should be returned along side the data.", True
    ...
    def publish_data_product(
        ...
        config: Optional[MtnaRdsPostmanPublisherConfig] = None,
    ) -> str:
    ...
    def _publish_data_product_in_item(
        ...
        item: list[Union[postman_collection.Item, postman_collection.ItemGroup]],
    ) -> None:
        ...
        metadata_rds_request_item.request.url.create_variable(
            ...
            if self.server.api_key:
                try:
                    ...
                    if mtna_collection_data:
                        ...
                        mtna_folder = templates.get_mtnards_collection_folder(
                            markdown=mtna_collection.info.description.content
                        )
