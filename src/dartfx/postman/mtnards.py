...
from dartfx.mtnards import MtnaRdsDataProduct, MtnaRdsServer
from dartfx.postmanapi import PostmanApi, WorkspaceManager, postman_collection

from . import templates
from ._postman_types import (
    as_collection,
    create_item_request,
    ensure_collection_info,
    ensure_item_request,
    ensure_request_url,
)

FORMAT_DESCRIPTION = (
    "RDS can return a variety of JSON objects to plug into various java script charting libraries. "
    "Available formats: MTNA_RDS, MTNA_simple, MTNA_TABULATE, AMCHARTS, GCHARTS, "
    "PLOTLY_BAR, PLOTLY_BOXPLOT, PLOTLY_BUBBLE, PLOTLY_HIST, PLOTLY_PIE, PLOTLY_SCATTER."
)
GROUPBY_DESCRIPTION = (
    "When computing a new variable using a function that depends on aggregation, the group by "
...
WHERE_DESCRIPTION = (
    ...
    "This follows a syntax similar to a SQL where clause."
)
SELECT_COLS_DESCRIPTION = (
    "Column names, regular expressions, keywords, variable groups, or concepts to select. "
    "Any of these can be excluded as well by prepending '~' to these syntaxes."
)
LOCK_DESCRIPTION = (
    "Column names, regular expressions, key words, variable groups, or concepts to lock. "
    "These will be returned first adn listed out in DataSetInformation that is returned on the DataSet."
)
VARIABLES_DESCRIPTION = (
    "Get the variables of a specific data product. Returns a list of summary objects about any variables "
    "that are used in the specified data product. These summaries contain both the variable URI and ID, "
    "which can be used to get more detailed information about the variable."
)
VARIABLE_DETAILS_DESCRIPTION = (
    "Returns a variable with more detail than what is provided by the summary object. If the variable has "
    "a classification, its URI should be available on the variable which can be used to retrieve the "
    "classification."
)
CLASSIFICATIONS_DESCRIPTION = (
    "Get the classifications of a specific data product. Returns a list of summary objects about any "
    "classifications that are used in the specified data product. These summary objects hold the "
    "classification uri and classification id (among other things) either of which can be used to get "
    "more information about the classification."
)
CLASSIFICATION_DETAILS_DESCRIPTION = (
    "Returns the specified classification with more detail than the summary. Codes will be excluded from "
    "this object, the idea being that this classification could have a large amount of codes and clients "
    "can build these codes up through the used of the codes endpoints."
)
CHANGELOG_DESCRIPTION = (
    "Get the change log for a data product. This will be an array of ChangeLog entries ordered with the "
    "latest first. If there is no changelog available an empty array will be returned."
)
LIST_GENERATORS_DESCRIPTION = (
    "Snippet generators can be used to create calls in external libraries that when run, will get the "
    "data set represented by a provided query. This end point exposes the available snippet generators "
    "and their details."
)
...
class MtnaRdsPostmanPublisherConfig(BaseModel):
    ...
    include_rds_collection: bool = Field(
        default=False
    )  # includes the RDS generated Postman collection (requires management API key)
...
class MtnaRdsPostmanPublisher(BaseModel):
    ...
    def publish_rds_collection_to_collection(
        self, catalog_id, data_product_id, collection_uid, folder_uid=None, temp_workspace_id=None, create_root=True
    ):
        ...
        target_model = (
            "folder" if folder_uid or create_root else "collection"
        )  # collection would be unusual but supported anyway
    ...
    def _create_folder_for_rds_collection(
        self, rds_collection: dict, collection_id, folder_id=None, name_prefix=None, name=None, description=None
    ):
    ...
    def _add_regression_request_parameters(self, item: postman_collection.Item):
        ...
        request_url.create_query_parameter("groupby", None, GROUPBY_DESCRIPTION, True)
        request_url.create_query_parameter(
            "inject", "1", "Flag specifying to inject codes into the returned records.", True
        )
        request_url.create_query_parameter("limit", "50", "The maximum number of records to return.", True)
        request_url.create_query_parameter(
            "metadata", "1", "Flag specifying if metadata should be returned along side the data.", True
        )
        request_url.create_query_parameter("offset", "0", "The record to start at.", True)
        request_url.create_query_parameter("orderby", None, ORDERBY_DESCRIPTION, True)
        request_url.create_query_parameter(
            "weights", None, "The IDs of variables to use as weights in the resulting data.", True
        )
        request_url.create_query_parameter("where", None, WHERE_DESCRIPTION, True)

    def _add_select_request_parameters(self, item: postman_collection.Item):
        request_url = ensure_request_url(ensure_item_request(item))
        request_url.create_query_parameter("collimit", "50", "Limits the number of columns returned.", True)
        request_url.create_query_parameter("coloffset", "0", "Determines which column to start at.", True)
        request_url.create_query_parameter("cols", None, SELECT_COLS_DESCRIPTION, True)
        request_url.create_query_parameter(
            "count", "0", "Flag specifying whether the total row count should be returned along side the data.", True
        )
        request_url.create_query_parameter(
            "format",
            None,
        ...
        request_url.create_query_parameter("limit", "50", "The maximum number of records to return.", True)
        request_url.create_query_parameter("lock", None, LOCK_DESCRIPTION, True)
        request_url.create_query_parameter(
            "metadata", "1", "Flag specifying if metadata should be returned along side the data.", True
        )
        request_url.create_query_parameter("offset", "0", "The record to start at.", True)
    ...
    def _add_tabulate_request_parameters(self, item: postman_collection.Item):
        request_url = ensure_request_url(ensure_item_request(item))
        request_url.create_query_parameter(
            "dims", None, "Required. Categorical variables / columns to use as dimensions.", False
        )
        request_url.create_query_parameter(
            "measure", None, "Columns to use as measures. Count is used by default.", True
        )
        request_url.create_query_parameter(
            "count", "1", "Flag specifying whether the total row count should be returned along side the data.", True
        )
        ...
        request_url.create_query_parameter("orderby", None, ORDERBY_DESCRIPTION, True)
        request_url.create_query_parameter(
            "totals", "1", "Flag specifying whether subtotals should be returned along side the data.", True
        )
        request_url.create_query_parameter(
            "weights", None, "The IDs of variables to use as weights in the resulting data.", True
        )
    ...
    def _add_snippet_generator_parameters(self, item: postman_collection.Item):
        request_url = ensure_request_url(ensure_item_request(item))
        request_url.create_query_parameter(
            "library", "rds-js", "The code generator library to use: rds-js, rds-python, rds-r", False
        )
        request_url.create_query_parameter(
            "version", "1.0.0-ALPHA", "the library's version (see snippet generators information endpoint)", False
        )

    def publish_catalog(self, _catalog_id, _workspace_id):
    ...
    def publish_data_product(
        ...
        config: MtnaRdsPostmanPublisherConfig | None = None,
    ) -> str:
        ...
        If both workspace_id and collection_id are provided, collection_id takes precedence,
        and the collection will be replaced.
        ...
        # name
        name = (
            f"{config.name_prefix or ''}{data_product.name} "
            f"[{data_product._catalog.id}/{data_product.id}]"
            f"{config.name_suffix or ''}"
        )
        collection_info.name = name

        # description
        collection_info.description = templates.get_collection_description(markdown=data_product.get_markdown())
        ...
        collection.variable.append(
            postman_collection.Variable(
                id="hvdnetUri", value=f"mtnards:{self.server.hostname}:{data_product.catalog_id}:{data_product.id}"
            )
        )
    ...
    def _publish_data_product_in_item(
        self,
        data_product: MtnaRdsDataProduct,
        config: MtnaRdsPostmanPublisherConfig,
        item: list[postman_collection.Item | postman_collection.ItemGroup],
    ) -> None:
        ...
        metadata_rds_request_item.description = VARIABLES_DESCRIPTION
        ...
        metadata_rds_request_item.description = VARIABLE_DETAILS_DESCRIPTION
        metadata_rds_request = create_item_request(
            metadata_rds_request_item,
            f"{data_product.metadata_api_url}/variable/:id",
        )
        ensure_request_url(metadata_rds_request).create_variable(
            key="id",
            description="The ID or URI of the variable.",
        )
        metadata_rds_folder.item.append(metadata_rds_request_item)

        metadata_rds_request_item = postman_collection.Item()
        metadata_rds_request_item.name = "Classifications"
        metadata_rds_request_item.description = CLASSIFICATIONS_DESCRIPTION
        ...
        metadata_rds_request_item.description = CLASSIFICATION_DETAILS_DESCRIPTION
        metadata_rds_request = create_item_request(
            metadata_rds_request_item,
            f"{data_product.metadata_api_url}/classification/:id",
        )
        ensure_request_url(metadata_rds_request).create_variable(
            key="id", description="The ID or URI of the classification."
        )
        metadata_rds_folder.item.append(metadata_rds_request_item)

        metadata_rds_request_item = postman_collection.Item()
        metadata_rds_request_item.name = "Classification Codes"
        metadata_rds_request_item.description = (
            "This allows the client to page through or build up the codes of the classification as desired."
        )
        metadata_rds_request = create_item_request(
            metadata_rds_request_item,
            f"{data_product.metadata_api_url}/classification/:id/codes",
        )
        ensure_request_url(metadata_rds_request).create_variable(
            key="id", description="The ID or URI of the classification."
        )
        ...
        metadata_rds_request_item.description = CHANGELOG_DESCRIPTION
        ...
        self._add_regression_request_parameters(request_item)
        data_folder.item.append(request_item)
        ...
        code_request_item.description = LIST_GENERATORS_DESCRIPTION
        ...
        code_request_item.description = (
            "Generates a snippet of code in the requested library to recreate the provided select query."
        )
        ...
        code_request_item.description = (
            "Generates a snippet of code in the requested library to recreate the provided tabulate query."
        )
            ...
            if self.server.api_key:
                try:
                    ...
                    if mtna_collection_data:
                        ...
                        mtna_collection = as_collection(postman_collection.Collection.from_dict(mtna_collection_data))
                        # Remove request ids; otherwise import through the Postman API fails.
                        # This does not seem to be an issue with request event scripts
                        for mtna_folder_item in mtna_collection.item:
                            if not isinstance(mtna_folder_item, postman_collection.ItemGroup):
                                continue
                            for mtna_request in mtna_folder_item.item:
                                if isinstance(mtna_request, postman_collection.Item):
                                    mtna_request.id = None
                        # create and populate folder
                        mtna_collection_info = ensure_collection_info(mtna_collection)
                        description = mtna_collection_info.description
                        markdown = (
                            description.content
                            if isinstance(description, postman_collection.Description)
                            else description
                        )
                        mtna_folder = templates.get_mtnards_collection_folder(markdown=markdown or "")
