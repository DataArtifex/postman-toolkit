...
class SocrataPostmanCollectionGenerator(BaseModel):
    ...
    def _add_query_request_parameters(self, request: postman_collection.Request):
        ...
        param.value = ",".join(
            self.dataset.get_visible_variables_names()
        )  # make sure only visible variables are selected by default
    ...
    def generate(self) -> postman_collection.Collection:
        ...
        # Metadata requests
        metadata_folder.item.append(
            templates.get_hvdnet_croissant_request(hvdnet_base_url, topic=urllib.parse.quote(f"{topic} [croissant]"))
        )
        metadata_folder.item.append(
            templates.get_hvdnet_dcat_request(hvdnet_base_url, topic=urllib.parse.quote(f"{topic} [dcat/json]"))
        )
        metadata_folder.item.append(
            templates.get_hvdnet_dcat_request(
                hvdnet_base_url, format="turtle", topic=urllib.parse.quote(f"{topic} [dcat/ttl]")
            )
        )
        metadata_folder.item.append(
            templates.get_hvdnet_ddi_codebook_request(hvdnet_base_url, topic=urllib.parse.quote(f"{topic} [ddi-c]"))
        )
        metadata_folder.item.append(
            templates.get_hvdnet_ddi_cdif_request(hvdnet_base_url, topic=urllib.parse.quote(f"{topic} [ddi-cdif/json]"))
        )
        metadata_folder.item.append(
            templates.get_hvdnet_ddi_cdif_request(
                hvdnet_base_url, format="turtle", topic=urllib.parse.quote(f"{topic} [ddi-cdif/ttl]")
            )
        )
        metadata_folder.item.append(
            templates.get_hvdnet_socrata_request(hvdnet_base_url, topic=urllib.parse.quote(f"{topic} [socrata]"))
        )
        ...
        ai_folder.item.append(
            templates.get_markdown_request(hvdnet_base_url, topic=urllib.parse.quote(f"{topic} [md]"))
        )
        ...
        collection.variable.append(
            postman_collection.Variable(id="hvdnetUri", value=f"socrata:{dataset.server.host}:{dataset.id}")
        )
